...
def get_params_groups(model, weight_decay=1e-5):
    param_group_vars = {"decay": {"params": [], "weight_decay": weight_decay},
                        "no_decay": {"params": [], "weight_decay": 0.}}
    param_group_names = {"decay": {"params": [], "weight_decay": weight_decay},
                         "no_decay": {"params": [], "weight_decay": 0.}}

    for name, params in model.named_parameters():
        if not params.requires_grad:
            continue
        if len(params.shape) == 1 or name.endswith(".bias"):
            group_name = "no_decay"
        else:
            group_name = "decay"
        param_group_vars[group_name]["params"].append(params)
